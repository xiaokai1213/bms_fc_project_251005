...
// 定义事件枚举
typedef enum {
   EVENT_BUTTON_PRESS,  // 按钮按下事件
   EVENT_TIMER_TICK     // 定时器事件
} Event;
...
typedef struct {
   ...
   int brightness;       // 亮度值
   bool blink_state;     // 闪烁状态(亮/灭)
} StateMachine;

// 初始化状态机
void sm_init(StateMachine* sm) {
   sm->current_state = STATE_OFF;  // 初始状态为关闭
   sm->brightness = 0;
   sm->blink_state = false;
}
   ...
   switch (sm->current_state) {
      ...
      case STATE_ON:
         ...
         switch (event) {
            case EVENT_BUTTON_PRESS:
               // 按钮按下，切换到闪烁状态
               sm->current_state = STATE_BLINK;
               sm->blink_state = true;  // 初始闪烁状态为亮
               printf("从开启状态切换到闪烁状态\n");
               break;
...
int main() {
   ...
   printf("\n--- 按钮按下事件 ---\n");
   sm_handle_event(&sm, EVENT_BUTTON_PRESS);

   printf("\n--- 定时器事件 ---\n");
   sm_handle_event(&sm, EVENT_TIMER_TICK);
