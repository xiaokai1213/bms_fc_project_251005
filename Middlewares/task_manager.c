...
#include <string.h>
#include "main.h"  // 使用全局系统时间变量 sys_time
 ...
 * @retval int        返回任务 ID（>=0）表示注册成功；返回 -1 表示失败（无可用插槽）
 */
int TM_Register(TaskFunc func, void* arg, uint32_t period_ms, uint8_t repeat, uint32_t delay_ms) {
   ...
   for (int i = 0; i < TASK_MANAGER_MAX_TASKS; i++) {
      if (!tm.used[i]) {
         ...
         /* 计算首次运行时间：现在 + delay_ms（允许 delay_ms 为0） */
         tm.tasks[i].next_run = sys_time + (uint64_t)delay_ms;
 ...
 */
void TM_Run(void) {
   uint64_t now = sys_time; /* 使用 sys_time 作为时间基准（ms） */
   for (int i = 0; i < TASK_MANAGER_MAX_TASKS; i++) {
      if (!tm.used[i])
         continue;
      ...
      if (now >= t->next_run) {
         t->func(t->arg);
         if (t->repeat && t->period_ms > 0) {
            /* 周期性任务：在原 next_run 上累加 period，避免时间漂移 */
            t->next_run += t->period_ms;
            /* 若系统太慢导致多次周期被跳过，则将 next_run 调整为 now+period_ms */
            if (now >= t->next_run) {
               t->next_run = now + t->period_ms;
            }
         } else {
            /* 一次性任务：执行后释放槽 */
            tm.used[i] = 0;
